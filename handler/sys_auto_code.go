...
func CreateTemp() (err error) {
	...
	for _, tpl := range goTpls {
		...
		model, err := os.OpenFile(tpl.Package+tpl.FileName, os.O_CREATE|os.O_WRONLY, 0755)
		if err != nil {
			global.GG_LOG.Info("GO文件打开失败:"+tpl.Package+tpl.FileName, err)
			continue
		}
		err = modelTmpl.Execute(model, autoCode)
		if err != nil {
			global.GG_LOG.Info("GO文件生成失败:"+tpl.Package+tpl.FileName, err)
			continue
		}
		_ = model.Close()
		global.GG_LOG.Info("成功创建:" + tpl.FileName)
	}
	return nil
}
